...
export function shouldBehaveLikeERC721Enumerable(module: MODULE) {
  context("with minted tokens", function () {
    ...
    describe("tokenOfOwnerByIndex", function () {
      describe("when the given index is lower than the amount of tokens owned by the given address", function () {
        it("returns the token ID placed at the given index", async function () {
          expect(
            await this.tokenAsEnumerableGetter.callStatic.tokenOfOwnerByIndex(this.signers.owner.address, 0),
          ).to.equal(firstTokenId);
        });
      });
      ...
      describe("after transferring all tokens to another user", function () {
        beforeEach(async function () {
          await this.tokenAsTransfer
            .connect(this.signers.owner)
            .transferFrom(this.signers.owner.address, this.signers.other.address, firstTokenId);
          await this.tokenAsTransfer
            .connect(this.signers.owner)
            .transferFrom(this.signers.owner.address, this.signers.other.address, secondTokenId);
        });

        it("returns correct token IDs for target", async function () {
          expect(await this.tokenAsBaseGetter.callStatic.balanceOf(this.signers.other.address)).to.equal(
            BigNumber.from(2),
          );
    ...
    describe("tokenByIndex", function () {
      ...
      it("reverts if index is greater than supply", async function () {
        await expect(this.tokenAsEnumerableGetter.tokenByIndex(2)).to.be.revertedWith(
  ...
  describe("_burn", function () {
    ...
    it("reverts when burning a non-existent token id", async function () {
      await expect(this.tokenAsBurn.burn(firstTokenId)).to.be.revertedWith("ERC721: owner query for nonexistent token");
    });
    ...
    context("with minted tokens", function () {
      ...
      context("with burnt token", function () {
        ...
        it("burns all tokens", async function () {
          ...
          await expect(this.tokenAsEnumerableGetter.tokenByIndex(0)).to.be.revertedWith(
            "ERC721Enumerable: global index out of bounds",
          );
        });
      });
