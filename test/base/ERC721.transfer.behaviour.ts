...
import { ERC721ReceiverMock, TransferLogic } from "../../src/types";
  ...
  context("with minted tokens", async function () {
    ...
    describe("transfers", function () {
      ...
      beforeEach(async function () {
        ...
        await expect(
          this.tokenAsApprove.connect(this.signers.owner).setApprovalForAll(this.signers.operator.address, true),
        ).to.not.be.reverted;
      });
      ...
      const transferWasSuccessful = function (owner: string, tokenId: BigNumber) {
        ...
        it("emits a Transfer event", async function () {
          await expectEvent(tx, "Transfer", { from: owner, to: this.toWhom, tokenId });
        });
        ...
        it("emits an Approval event", async function () {
          await expectEvent(tx, "Approval", { _owner: owner, _approved: ZERO_ADDRESS, _tokenId: tokenId });
        });
        ...
        it("adjusts owners tokens by index", async function () {
          ...
          expect(await this.tokenAsEnumerableGetter.tokenOfOwnerByIndex(this.signers.owner.address, 0)).to.not.equal(
            tokenId,
          );
        });
      };

      const shouldTransferTokensByUsers = async function (
        transferFunction: (
          tokenAsTransfer: TransferLogic,
        ...
        context("when called by the owner without an approved user", function () {
          it("should succeed", async function () {
            await expect(
              this.tokenAsApprove.connect(this.signers.owner).approve(ZERO_ADDRESS, firstTokenId),
            ).to.not.be.reverted;
            await expect(
              transferFunction(
                this.tokenAsTransfer,
        ...
        context("when sent to the owner", function () {
          ...
          it("keeps ownership of the token", async function () {
            expect(await this.tokenAsBaseGetter.callStatic.ownerOf(firstTokenId)).to.be.equal(
              this.signers.owner.address,
            );
          });
          ...
          it("keeps same tokens by index", async function () {
            ...
            const tokensListed = await Promise.all(
              [0, 1].map(i =>
                this.tokenAsEnumerableGetter.callStatic.tokenOfOwnerByIndex(this.signers.owner.address, i),
              ),
            );
      ...
      describe("via safeTransferFrom", async function () {
        it("with data", async function () {
          ...
          await shouldTransferSafely(safeTransferFromWithData, data);
        });
        ...
        it("without data", async function () {
          ...
          await shouldTransferSafely(safeTransferFromWithoutData, null);
        });

        const shouldTransferSafely = async function (
          transferFunction: (
            token: TransferLogic,
            signer: SignerWithAddress,
            from: string,
          ...
          describe("to a valid receiver contract", async function () {
            beforeEach(async function () {
              this.erc721Receiver = await (<ERC721ReceiverMock>(
                await waffle.deployContract(this.signers.admin, this.artifacts.erc721Receiver, [
        ...
        describe("to a receiver contract returning unexpected value", function () {
          it("reverts", async function () {
            const invalidReceiver = await (<ERC721ReceiverMock>(
              await waffle.deployContract(this.signers.admin, this.artifacts.erc721Receiver, ["0x42424242", Error.None])
            ));
            await expect(
              this.tokenAsTransfer
        ...
        describe("to a receiver contract that reverts with message", function () {
          it("reverts", async function () {
            const revertingReceiver = await (<ERC721ReceiverMock>(
              await waffle.deployContract(this.signers.admin, this.artifacts.erc721Receiver, [
                RECEIVER_MAGIC_VALUE,
        ...
        describe("to a contract that does not implement the required function", function () {
          it("reverts", async function () {
            ...
            await expect(
              this.tokenAsTransfer
                .connect(this.signers.owner)
                ["safeTransferFrom(address,address,uint256)"](this.signers.owner.address, nonReceiver.address, tokenId),
            ).to.be.revertedWith("ERC721: transfer to non ERC721Receiver implementer");
          });
        });
      });
