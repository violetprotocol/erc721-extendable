  ...
  context("approve with minted tokens", async function () {
    ...
    describe("approve", function () {
      ...
      const itEmitsApprovalEvent = function (address: any) {
        it("emits an approval event", async function () {
          await expectEvent(tx, "Approval", {
            _owner: this.signers.owner.address,
            _approved: address,
            _tokenId: tokenId,
          });
        });
      ...
      context("when approving a non-zero address", function () {
        ...
        context("when there was a prior approval to a different address", function () {
          beforeEach(async function () {
            await this.tokenAsApprove
              .connect(this.signers.owner)
              .approve(this.signers.anotherApproved.address, tokenId);
            tx = await this.tokenAsApprove
              .connect(this.signers.owner)
              .approve(this.signers.anotherApproved.address, tokenId);
          });
      ...
      context("when the sender does not own the given token ID", function () {
        it("reverts", async function () {
          await expect(
            this.tokenAsApprove.connect(this.signers.other).approve(this.signers.approved.address, tokenId),
      ...
      context("when the sender is approved for the given token ID", function () {
        it("reverts", async function () {
          await this.tokenAsApprove.connect(this.signers.owner).approve(this.signers.approved.address, tokenId);
          await expect(
            this.tokenAsApprove.connect(this.signers.approved).approve(this.signers.anotherApproved.address, tokenId),
        ...
        it("reverts", async function () {
          await expect(
            this.tokenAsApprove
              ...
              .approve(this.signers.approved.address, nonExistentTokenId),
    ...
    describe("setApprovalForAll", function () {
      context("when the operator willing to approve is not the owner", function () {
        context("when there is no operator approval set by the sender", function () {
          it("approves the operator", async function () {
            await this.tokenAsApprove
              .connect(this.signers.owner)
              .setApprovalForAll(this.signers.operator.address, true);

            expect(
              await this.tokenAsBaseGetter.callStatic.isApprovedForAll(
          ...
          it("emits an approval event", async function () {
            const tx = await this.tokenAsApprove
              .connect(this.signers.owner)
              .setApprovalForAll(this.signers.operator.address, true);

            await expectEvent(tx, "ApprovalForAll", {
              _owner: this.signers.owner.address,
              _operator: this.signers.operator.address,
              _approved: true,
            });
          });
        });
        ...
        context("when the operator was set as not approved", function () {
          beforeEach(async function () {
            await this.tokenAsApprove
              .connect(this.signers.owner)
              .setApprovalForAll(this.signers.operator.address, false);
          });
      ...
      context("when the operator is the owner", function () {
        it("reverts", async function () {
          await expect(
            this.tokenAsApprove.connect(this.signers.owner).setApprovalForAll(this.signers.owner.address, true),
    ...
    describe("getApproved", async function () {
      context("when token is not minted", async function () {
        it("reverts", async function () {
          await expect(this.tokenAsBaseGetter.getApproved(nonExistentTokenId)).to.be.revertedWith(
