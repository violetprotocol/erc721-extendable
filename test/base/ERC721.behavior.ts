...
import { TOKEN_NAME, TOKEN_SYMBOL } from "./ERC721.test";
...
const shouldBehaveLikeERC721 = () => {
  ...
  before(async function() {
    erc721 = <Extended<ERC721>>await deployExtendableContract(this.signers.admin, this.artifacts.erc721, [TOKEN_NAME, TOKEN_SYMBOL, this.extend.address, this.approve.address, this.baseGetter.address, this.onReceive.address, this.transfer.address, this.beforeTransfer.address]) 
    ...
    this.tokenAsMint = <BasicMintLogic>await this.erc721.as(this.artifacts.mint);
  ...
  context('with minted tokens', async function () {
    beforeEach(async function () {
      ...
      this.toWhom = this.other; // default to other for toWhom in context-dependent tests
    });

    describe('balanceOf', function () {
      context('when the given address owns some tokens', function () {
        it('returns the amount of tokens owned by the given address', async function () {
          console.log(await this.tokenAsBaseGetter.balanceOf(this.signers.owner.address));
          expect(await this.tokenAsBaseGetter.balanceOf(this.signers.owner.address)).to.be.bignumber.equal('2');
        });
      });

      context('when the given address does not own any tokens', function () {
        it('returns 0', async function () {
          expect(await this.tokenAsBaseGetter.balanceOf(this.other)).to.be.bignumber.equal('0');
        });
      });
      ...
      context('when querying the zero address', function () {
        it('throws', async function () {
          await expectRevert(
            this.tokenAsBaseGetter.balanceOf(ZERO_ADDRESS), 'ERC721: address zero is not a valid owner',
          );
        });
      });
    });
    ...
    describe('transfers', function () {
      ...
      const transferWasSuccessful = function (owner: string, tokenId: number) {
        ...
        it('adjusts owners balances', async function () {
          expect(await this.tokenAsBaseGetter.balanceOf(owner)).to.be.bignumber.equal('1');
        });
      ...
      const shouldTransferTokensByUsers = function (transferFunction: any) {
        ...
        context('when called by the owner without an approved user', function () {
          beforeEach(async function () {
            await this.token.approve(ZERO_ADDRESS, tokenId, { from: this.signers.owner.address });
            ({ logs } = await transferFunction.call(this, this.signers.owner.address, this.toWhom, tokenId, { from: this.operator }));
          });
          it("", async function () {
            transferWasSuccessful( this.signers.owner.address, tokenId);
        ...
        context('when sent to the owner', function () {
          ...
          it('keeps ownership of the token', async function () {
            expect(await this.token.ownerOf(tokenId)).to.be.equal(this.signers.owner.address);
          });
          ...
          it('keeps the owner balance', async function () {
            expect(await this.token.balanceOf(this.signers.owner.address)).to.be.bignumber.equal('2');
          });
        ...
        context('when the address of the previous owner is incorrect', function () {
          it('reverts', async function () {
            await expectRevert(
              transferFunction.call(this, this.other, this.other, tokenId, { from: this.signers.owner.address }),
        ...
        context('when the sender is not authorized for the token id', function () {
          it('reverts', async function () {
            await expectRevert(
              transferFunction.call(this, this.signers.owner.address, this.other, tokenId, { from: this.other }),
        ...
        context('when the given token ID does not exist', function () {
          it('reverts', async function () {
            await expectRevert(
              transferFunction.call(this, this.signers.owner.address, this.other, nonExistentTokenId, { from: this.signers.owner.address }),
      ...
      describe('via safeTransferFrom', function () {
        ...
        describe('to a receiver contract returning unexpected value', function () {
          it('reverts', async function () {
            ...
            await expectRevert(
              this.token.safeTransferFrom(this.signers.owner.address, invalidReceiver.address, tokenId, { from: this.signers.owner.address }),
        ...
        describe('to a receiver contract that reverts with message', function () {
          it('reverts', async function () {
            ...
            await expectRevert(
              this.token.safeTransferFrom(this.signers.owner.address, revertingReceiver.address, tokenId, { from: this.signers.owner.address }),
    ...
    describe('safe mint', function () {
      ...
      describe('via safeMint', function () { // regular minting is tested in ERC721Mintable.test.js and others
        it('calls onERC721Received — with data', async function () {
          ...
          const receipt = await this.tokenAsMint.safeMint(this.receiver.address, tokenId, data);
        ...
        it('calls onERC721Received — without data', async function () {
          this.receiver = await <ERC721ReceiverMock>(await waffle.deployContract(this.signers.admin, this.artifacts.erc721Receiver, [RECEIVER_MAGIC_VALUE, Error.None]));
          const receipt = await this.tokenAsMint.safeMint(this.receiver.address, tokenId);
        ...
        context('to a receiver contract returning unexpected value', function () {
          it('reverts', async function () {
            const invalidReceiver = await <ERC721ReceiverMock>(await waffle.deployContract(this.signers.admin, this.artifacts.erc721Receiver, ['0x42', Error.None]));
            await expectRevert(
              this.tokenAsMint.safeMint(invalidReceiver.address, tokenId),
              'ERC721: transfer to non ERC721Receiver implementer',
            );
          });
        });
        ...
        context('to a receiver contract that reverts with message', function () {
          it('reverts', async function () {
            const revertingReceiver = await <ERC721ReceiverMock>(await waffle.deployContract(this.signers.admin, this.artifacts.erc721Receiver, [RECEIVER_MAGIC_VALUE, Error.RevertWithMessage]));
            await expectRevert(
              this.tokenAsMint.safeMint(revertingReceiver.address, tokenId),
              'ERC721ReceiverMock: reverting',
            );
          });
        });
        ...
        context('to a contract that does not implement the required function', function () {
          it('reverts', async function () {
            const nonReceiver = this.token;
            await expectRevert(
              this.tokenAsMint.safeMint(nonReceiver.address, tokenId),
              'ERC721: transfer to non ERC721Receiver implementer',
            );
          });
        });
    ...
    describe('approve', function () {
      ...
      context('when the sender does not own the given token ID', function () {
        it('reverts', async function () {
          await expectRevert(this.tokenAsApprove.approve(this.approved, tokenId, { from: this.other }),
      ...
      context('when the given token ID does not exist', function () {
        it('reverts', async function () {
          await expectRevert(this.token.approve(this.approved, nonExistentTokenId, { from: this.operator }),
    ...
    describe('setApprovalForAll', function () {
      context('when the operator willing to approve is not the owner', function () {
        context('when there is no operator approval set by the sender', function () {
          it('approves the operator', async function () {
            await this.token.setApprovalForAll(this.operator, true, { from: this.signers.owner.address });

            expect(await this.token.isApprovedForAll(this.signers.owner.address, this.operator)).to.equal(true);
          });

          it('emits an approval event', async function () {
            const { logs } = await this.token.setApprovalForAll(this.operator, true, { from: this.signers.owner.address });
        ...
        context('when the operator was set as not approved', function () {
          beforeEach(async function () {
            await this.token.setApprovalForAll(this.operator, false, { from: this.signers.owner.address });
          });
          ...
          it('can unset the operator approval', async function () {
            ...
            expect(await this.token.isApprovedForAll(this.signers.owner.address, this.operator)).to.equal(false);
          });
        });
      ...
      context('when the operator is the owner', function () {
        it('reverts', async function () {
          await expectRevert(this.token.setApprovalForAll(this.signers.owner.address, true, { from: this.signers.owner.address }),
    ...
    describe('getApproved', async function () {
      ...
      context('when token has been minted ', async function () {
        ...
        context('when account has been approved', async function () {
          beforeEach(async function () {
            await this.token.approve(this.approved, firstTokenId, { from: this.signers.owner.address });
          });

          it('returns approved account', async function () {
            expect(await this.token.getApproved(firstTokenId)).to.be.equal(this.approved);
          });
        });
      });
    });
  ...
  describe('_mint(address, uint256)', function () {
    it('reverts with a null destination address', async function () {
      await expectRevert(
        this.tokenAsMint.mint(ZERO_ADDRESS, firstTokenId), 'ERC721: mint to the zero address',
      );
    });

    context('with minted token', async function () {
      beforeEach(async function () {
        ({ logs: this.logs } = await this.tokenAsMint.mint(this.signers.owner.address, firstTokenId));
      });
      ...
      it('creates the token', async function () {
        expect(await this.token.balanceOf(this.signers.owner.address)).to.be.bignumber.equal('1');
        expect(await this.token.ownerOf(firstTokenId)).to.equal(this.signers.owner.address);
      });

      it('reverts when adding a token id that already exists', async function () {
        await expectRevert(this.tokenAsMint.mint(this.signers.owner.address, firstTokenId), 'ERC721: token already minted');
      });
    });
  });
  ...
  describe('_burn', function () {
    it('reverts when burning a non-existent token id', async function () {
      await expectRevert(
        this.tokenAsBurn.burn(nonExistentTokenId), 'ERC721: owner query for nonexistent token',
      );
    });
    ...
    context('with minted tokens', function () {
      ...
      context('with burnt token', function () {
        beforeEach(async function () {
          ({ logs: this.logs } = await this.tokenAsBurn.burn(firstTokenId));
        });
        ...
        it('reverts when burning a token id that has been deleted', async function () {
          await expectRevert(
            this.tokenAsBurn.burn(firstTokenId), 'ERC721: owner query for nonexistent token',
          );
        });
      });
    });
