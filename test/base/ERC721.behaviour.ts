...
const shouldBehaveLikeERC721 = () => {
    ...
    context('with minted tokens', async function () {
        ...
        describe('transfers', function () {
            ...
            const shouldTransferTokensByUsers = function (tokenMint: ERC721MockExtension, tokenApprove: ApproveLogic, tokenGetter: GetterLogic, transferFunction: (signer: SignerWithAddress, from: string, to: string, tokenId: BigNumber, opts?: any) => Promise<TransactionReceipt>) {
                beforeEach(async function () {
                    newTokenId++;
                    await expect(tokenMint.mint(this.signers.owner.address, BigNumber.from(newTokenId))).to.not.be.reverted;
                    await expect(tokenApprove.connect(this.signers.owner).approve(this.signers.approved.address, BigNumber.from(newTokenId))).to.not.be.reverted;
                });
                
                context('when called by the owner', function () {
                    it("should succeed", async function () {
                        tx = <ContractTransaction><any>await expect(transferFunction(this.signers.owner, this.signers.owner.address, this.signers.toWhom.address, BigNumber.from(newTokenId))).to.not.be.reverted;
                        transferWasSuccessful(this.signers.owner.address, BigNumber.from(newTokenId));
                    })
                });
        
                context('when called by the approved individual', function () {
                    it("should succeed", async function () {
                        tx = <ContractTransaction><any>await expect(transferFunction(this.signers.approved, this.signers.owner.address, this.signers.toWhom.address, BigNumber.from(newTokenId))).to.not.be.reverted;
                        transferWasSuccessful( this.signers.owner.address, BigNumber.from(newTokenId));
                    })
                });
                ...
                context('when called by the operator', function () {
                    it("should succeed", async function () {
                        await expect(transferFunction(this.signers.operator, this.signers.owner.address, this.signers.toWhom.address, BigNumber.from(newTokenId))).to.not.be.reverted;
                        transferWasSuccessful( this.signers.owner.address, BigNumber.from(newTokenId));
                    })
                });
                ...
                context('when called by the owner without an approved user', function () {
                    it("should succeed", async function () {
                        await expect(tokenApprove.connect(this.signers.owner).approve(ZERO_ADDRESS, BigNumber.from(newTokenId))).to.not.be.reverted;
                        await expect(transferFunction(this.signers.operator, this.signers.owner.address, this.signers.toWhom.address, BigNumber.from(newTokenId))).to.not.be.reverted;
                        transferWasSuccessful( this.signers.owner.address, BigNumber.from(newTokenId));
                    })
                });
                ...
                context('when sent to the owner', function () {
                    let tx: any;
                    beforeEach(async function () {
                        tx = await transferFunction(this.signers.owner, this.signers.owner.address, this.signers.owner.address, BigNumber.from(newTokenId));
                    });
            
                    it('keeps ownership of the token', async function () {
                        expect(await tokenGetter.callStatic.ownerOf(BigNumber.from(newTokenId))).to.be.equal(this.signers.owner.address);
                    });
            
                    it('clears the approval for the token ID', async function () {
                        expect(await tokenGetter.callStatic.getApproved(BigNumber.from(newTokenId))).to.be.equal(ZERO_ADDRESS);
                    });
                    ...
                    it('emits only a transfer event', async function () {
                        expectEvent(tx, 'Transfer', {
                            ...
                            tokenId: BigNumber.from(newTokenId),
                        });
                    });
            
                    it('keeps the owner balance', async function () {
                        expect(await tokenGetter.callStatic.balanceOf(this.signers.owner.address)).to.equal(BigNumber.from(newTokenId-2));
                    });
                ...
                context('when the address of the previous owner is incorrect', function () {
                    it('reverts', async function () {
                        await expect(transferFunction(this.signers.owner, this.signers.other.address, this.signers.other.address, BigNumber.from(newTokenId)))
                ...
                context('when the sender is not authorized for the token id', function () {
                    it('reverts', async function () {
                        await expect(transferFunction(this.signers.other, this.signers.owner.address, this.signers.other.address, BigNumber.from(newTokenId)))
                ...
                context('when the given token ID does not exist', function () {
                    it('reverts', async function () {
                        await expect(transferFunction(this.signers.owner, this.signers.owner.address, this.signers.other.address, nonExistentTokenId))
                ...
                context('when the address to transfer the token to is the zero address', function () {
                    it('reverts', async function () {
                        await expect(transferFunction(this.signers.owner, this.signers.owner.address, ZERO_ADDRESS, BigNumber.from(newTokenId)))
            ...
            describe('via transferFrom', function () {
                it("", async function() {
                    shouldTransferTokensByUsers(this.tokenAsErc721MockExtension, this.tokenAsApprove, this.tokenAsBaseGetter, (sender: SignerWithAddress, from: string, to: string, tokenId: BigNumber) => {
                        return this.tokenAsTransfer.connect(sender).transferFrom(from, to, tokenId);
                    });
                })
            ...
            describe('via safeTransferFrom', function () {
                ...
                //       const receipt = await transferFun.call(this, this.signers.owner.address, this.receiver.address, tokenId, { from: this.signers.approved.address });
            ...
            //       beforeEach(async function () {
            //         await this.tokenAsApprove.approve(this.signers.approved.address, tokenId, { from: this.signers.owner.address });
            //         ({ logs } = await this.tokenAsApprove.approve(ZERO_ADDRESS, tokenId, { from: this.signers.owner.address }));
            //       });
            ...
            //     context('when there was no prior approval', function () {
            //       beforeEach(async function () {
            //         ({ logs } = await this.tokenAsApprove.approve(this.signers.approved.address, tokenId, { from: this.signers.owner.address }));
            //       });
            ...
            //     it('reverts', async function () {
            //       await expectRevert(this.tokenAsApprove.approve(this.signers.approved.address, tokenId, { from: this.signers.other.address }),
            ...
            //     it('reverts', async function () {
            //       await this.tokenAsApprove.approve(this.signers.approved.address, tokenId, { from: this.signers.owner.address });
            //       await expectRevert(this.tokenAsApprove.approve(this.anotherApproved, tokenId, { from: this.signers.approved.address }),
            ...
            //     beforeEach(async function () {
            //       await this.tokenAsApprove.setApprovalForAll(this.signers.operator.address, true, { from: this.signers.owner.address });
            //       ({ logs } = await this.tokenAsApprove.approve(this.signers.approved.address, tokenId, { from: this.signers.operator.address }));
            //     });
            ...
            //   context('when the given token ID does not exist', function () {
            //     it('reverts', async function () {
            //       await expectRevert(this.tokenAsApprove.approve(this.signers.approved.address, nonExistentTokenId, { from: this.signers.operator.address }),
            ...
            //       it('emits an approval event', async function () {
            //         const { logs } = await this.tokenAsApprove.setApprovalForAll(this.signers.operator.address, true, { from: this.signers.owner.address });
            ...
            //       beforeEach(async function () {
            //         await this.tokenAsApprove.setApprovalForAll(this.signers.operator.address, false, { from: this.signers.owner.address });
            //       });
            ...
            //       beforeEach(async function () {
            //         await this.tokenAsApprove.approve(this.signers.approved.address, firstTokenId, { from: this.signers.owner.address });
            //       });
...
const expectEvent = async (tx: ContractTransaction, eventName: string, params: any) => {
