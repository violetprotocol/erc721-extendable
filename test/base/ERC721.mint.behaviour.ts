...
import { ERC721ReceiverMock } from "../../src/types";
          ...
          describe("via safeMint", function () {
            ...
            context("from owner", async function () {
              it("calls onERC721Received — with data", async function () {
                ...
                const tx = await this.tokenAsErc721MockExtension["safeMint(address,uint256,bytes)"](
              ...
              it("calls onERC721Received — without data", async function () {
                ...
                const tx = await this.tokenAsErc721MockExtension["safeMint(address,uint256)"](
                  this.erc721Receiver.address,
                  tokenId,
                );
            ...
            context("from non-owner", async function () {
              it("calls onERC721Received — with data fails", async function () {
                ...
                await expect(
                  this.tokenAsErc721MockExtension
                    .connect(this.signers.other)
                    ["safeMint(address,uint256,bytes)"](this.erc721Receiver.address, tokenId, data),
              ...
              it("calls onERC721Received — without data fails", async function () {
                ...
                await expect(
                  this.tokenAsErc721MockExtension
                    .connect(this.signers.other)
                    ["safeMint(address,uint256)"](this.erc721Receiver.address, tokenId),
                ).to.be.revertedWith("Logic: unauthorised");
              });
              ...
              context("to a receiver contract returning unexpected value", function () {
                it("reverts", async function () {
                  ...
                  await expect(
                    this.tokenAsErc721MockExtension
                      .connect(this.signers.other)
                      ["safeMint(address,uint256)"](invalidReceiver.address, tokenId),
                  ).to.be.revertedWith("Logic: unauthorised");
                });
              });
              ...
              context("to a contract that does not implement the required function", function () {
                it("reverts", async function () {
                  ...
                  await expect(
                    this.tokenAsErc721MockExtension
                      .connect(this.signers.other)
                      ["safeMint(address,uint256)"](nonReceiver.address, tokenId),
                  ).to.be.revertedWith("Logic: unauthorised");
                });
              });
            });
